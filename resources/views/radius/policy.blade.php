                                            ...
                                            <div class="alert-container"></div>
                                                <form id="{{ $attribute_name}}_form">
                                                    <div class="mb-2">
                                                        ...
                                                        <input type="text" id="name" class="form-control form-control-sm" placeholder="Type {{$page_name_small}} Name" required>
                                                        <input type="hidden" id="token" value="{{ $token }}"/>
                                                    </div>
                                                    <div class="mb-2">
                                                        <label for="recipient-name"
                                                            class="col-form-label">{{ $page_name_small}} Description</label>
                                                        <input type="text" id="desc" class="form-control form-control-sm" placeholder="Type Description" required>
                                                    </div>
                                            </div>
...
$('#datatable-basic').on('click', '.update', function() {
    ...
    let id = $(this).data("id");
            ...
            $.ajax({
            url: '/update_policy_modal/'+id, // Your API endpoint
...
$('#datatable-basic').on('click', '.alert-confirm', function() {
        ...
        Swal.fire({
            title: "Are you sure?",
            text: "This will also delete all associated group attributes. You won't be able to undo this action!",
            icon: "warning",
            ...
            if (e.isConfirmed) {
                ...
                let id = $(this).data("id");
                $.ajax({
                    url: baseUrl+'/policydelete/'+id, // replace with your API URL
                    method: 'POST',
                    data: {
...
$('#{{$attribute_name}}_form').on('submit', function (e) {
          ...
          $('#{{$attribute_name}}_form input, #{{$attribute_name}}_form button').prop('disabled', true);
            ...
            // Send AJAX request
            $.ajax({
                url: baseUrl+'/policyadd',  // Replace with your API endpoint
                type: 'POST',
                ...
                success: function (response) {
                    ...
                    if (response.status === 1) {
                        showToast("bg-success","Policy Added.",response.message)
...
$('#update_policy_form').on('submit', function (e) {
            e.preventDefault();


            // Disable all inputs and buttons
          $('#update_policy_form input, #update_policy_form button').prop('disabled', true);
            $('#updateBtn').hide();
            $('#updateloadingBtn').show();
            ...
            // Send AJAX request
            $.ajax({
                url: baseUrl+'/policyupdate/'+id,  // Replace with your API endpoint
                type: 'POST',
                data: JSON.stringify(formData),
                contentType: 'application/json',
                headers: {
                ...
                success: function (response) {
                    ...
                    if (response.status === 1) {
                        showToast("bg-success","Policy Updated.",response.message)
                ...
                $('#update_policy_form input, #update_policy_form button').prop('disabled', false);
                $('#updateloadingBtn').hide();
                $('#updateBtn').show();
                 }
            });
    ...
    load_datatable();
              ...
              $('#datatable-basic').DataTable({
                            ...
                            "pageLength": 10,
                            "ajax": {
                                "url": baseUrl+"/policy", // Replace with your API URL
                                "type": "GET",
                            ...
                            "columnDefs": [
                                ...
                                },
                                {
                                    ...
                                    "render": function(data, type, row, meta) {
                                        return `
                                            <a href="policy/grouplist/${row.id}" class="btn btn-primary-light btn-sm btn-wave">Group List <i class="bx bx-list-check"></i></a>
                                        `;
                                    }
                                },
                                {
                                    ...
                                    "render": function(data, type, row, meta) {
                                         ...
                                         if (row.totaluse == 0) {
                                            ...
                                            buttons += `
                                            <button type="button" data-id="${row.id}" class="btn btn-warning btn-sm btn-wave update">Edit</button>
                                            <button class="btn btn-danger btn-sm btn-wave" disabled>Delete</button>
                                            `;
                                        }
