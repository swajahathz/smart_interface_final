@php
    $page_name = "POLICY GROUP LIST";
    $page_name_small = "Group";
                    ...
                    <div class="col-xl-12">
                        <div class="card custom-card">
                            <div class="card-header d-flex justify-content-between">
                                ...
                                <div>
                                <button type="button" class="btn btn-primary btn-wave btn-sm"  data-bs-effect="effect-flip-vertical" data-bs-toggle="modal" href="#modaldemo8"><i class='bx bx-plus'></i> Add Attribute</button>
                                            ...
                                            <div class="alert-container"></div>
                                                <form id="{{ $attribute_name}}_form">
                                                    <div class="mb-2">
                                                        ...
                                                        <input type="text" id="groupname" class="form-control form-control-sm" value="{{ $groupname }}" readonly>
                                                    ...
                                                    <div class="mb-2">
                                                        ...
                                                        <input type="text" id="attribute" class="form-control form-control-sm" placeholder="Type Attribute" required>
                                                    </div>
                                                    <div class="mb-2">
                                                        <label for="recipient-name"
                                                            class="col-form-label">OP</label>
                                                        <input type="text" id="op" class="form-control form-control-sm" value="=" required>
                                                    </div>
                                                    <div class="mb-2">
                                                        <label for="recipient-name"
                                                            class="col-form-label">Value</label>
                                                        <input type="text" id="value" class="form-control form-control-sm" placeholder="Type Value" required>
                                                    </div>
                                            </div>
...
$('#datatable-basic').on('click', '.update', function() {
    ...
    let id = $(this).data("id");
            ...
            $.ajax({
            url: '/update_policy_group_modal/'+id, // Your API endpoint
...
$('#datatable-basic').on('click', '.alert-confirm', function() {
        ...
        Swal.fire({
            ...
            if (e.isConfirmed) {
                ...
                let id = $(this).data("id");
                $.ajax({
                    url: baseUrl+'/policygroupdelete/'+id, // replace with your API URL
                    method: 'POST',
                    data: {
...
$('#{{$attribute_name}}_form').on('submit', function (e) {
          ...
          $('#{{$attribute_name}}_form input, #{{$attribute_name}}_form button').prop('disabled', true);
            ...
            // Send AJAX request
            $.ajax({
                url: baseUrl+'/policygroupadd',  // Replace with your API endpoint
                type: 'POST',
                ...
                success: function (response) {
                    ...
                    if (response.status === 1) {
                        showToast("bg-success","attribute Added.",response.message)
...
$('#update_group_form').on('submit', function (e) {
            ...
            // Disable all inputs and buttons
          $('#update_group_form input, #update_group_form button').prop('disabled', true);
            $('#updateBtn').hide();
            $('#updateloadingBtn').show();
            ...
            let formData = {
                ...
                attribute: $('#update_attribute').val(),
                op: $('#update_op').val(),
            ...
            // Send AJAX request
            $.ajax({
                url: baseUrl+'/policygroupupdate/'+id,  // Replace with your API endpoint
                type: 'POST',
                data: JSON.stringify(formData),
                contentType: 'application/json',
                headers: {
                ...
                success: function (response) {
                    ...
                    if (response.status === 1) {
                        showToast("bg-success","Attribute Updated.",response.message)
                ...
                $('#update_group_form input, #update_group_form button').prop('disabled', false);
                $('#updateloadingBtn').hide();
                $('#updateBtn').show();
                 }
            });
    ...
    load_datatable();
              ...
              $('#datatable-basic').DataTable({
                            ...
                            "pageLength": 10,
                            "ajax": {
                                "url": baseUrl+"/policygrouplist/"+{{$id}}, // Replace with your API URL
                                "type": "GET",
                            ...
                            "columnDefs": [
                                ...
                                },
                                {
                                    ...
                                    "render": function(data, type, row, meta) {
                                        return `
                                            <a href="/policy" class="btn btn-primary-light btn-sm btn-wave">${row.groupname}</a>
                                        `;
                                    }
                                },
                                {
