            ...
            Your toast message here.
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
                        <h1 class="page-title fw-semibold fs-18 mb-0">CREATE MANAGER</h1>
                            ...
                            <div class="card custom-card">
                                    ...
                                    <div class="row gy-4 mb-4"> 
                                        <div class="col-xl-12">
                                           ...
                                           <div class="input-group">
                                                    @if($roles_id == 1)
                                                        <input type="text" class="form-control" value="Admin" id="type" readonly>
                                                    @elseif($roles_id == 2)
                                                        <input type="text" class="form-control" value="Franchise" id="type" readonly>    
                                                    @elseif($roles_id == 3)
                                                        <input type="text" class="form-control" value="Dealer" id="type" readonly>
                                                    @elseif($roles_id == 4)
                                                        <input type="text" class="form-control" value="SubDealer" id="type" readonly>
                                                    @elseif($roles_id == 5)
                                                        <input type="text" class="form-control" value="JuniorDealer" id="type" readonly>
                                                    @endif
                                            </div>
                                        </div>
                                        <div class="col-xl-6">
                                            <label for="username" class="form-label">Managername</label>
                                            <div class="input-group">
                                                    ...
                                                    <input type="hidden" class="form-control" id="roles_id" value="{{$roles_id}}" required>
                                                    <input type="hidden" class="form-control" id="user_id" value="{{$user_id}}" required>
                                                    <input type="hidden" class="form-control" id="user_name" value="{{$user_name}}" required>
                                                </div>
                                ...
                                <div class="card-footer text-end">
                                                <button type="submit" id="submitBtn" href="javascript:void(0);" class="btn btn-primary btn-wave waves-effect waves-light">
                                                    <i class="bi bi-plus-circle"></i> Create Manager
                                                </button>
...
$('#manager_form').on('submit', function (e) {
    ...
    let requiredFields = ['#username', '#password', '#firstname', '#lastname', '#address', '#mobile', '#phone', '#email', '#cnic'];
    let isValid = true;

    requiredFields.forEach(function (field) {
        if ($(field).val().trim() === '') {
            isValid = false;
            $(field).addClass('is-invalid'); // Highlight empty fields
        } else {
            $(field).removeClass('is-invalid'); // Remove highlight if filled
        }
    });
    ...
    $('#manager_form input, #manager_form button').prop('disabled', true);
    $('#submitBtn').hide();
    $('#loadingBtn').show();
    ...
    // Send AJAX request
    $.ajax({
        url: baseUrl + '/register',  // Replace with your API endpoint
        type: 'POST',
        ...
        success: function (response) {
            ...
            if (response.status === 1) {
                showToast("bg-success", "Manager Added.", response.message);
    ...
    showToast("bg-danger", "Validation Error!", errorMessages);
} else if (response.status === 501) {
                showToast("bg-danger", "Server error.", response.message);
            } else if ([500, 404].includes(response.status)) {
        ...
        complete: function () {
            ...
            $('#manager_form input, #manager_form button').prop('disabled', false);
            $('#loadingBtn').hide();
            $('#submitBtn').show();
        }
    });
