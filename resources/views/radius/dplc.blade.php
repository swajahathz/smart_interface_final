            ...
            Your toast message here.
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
                        <h1 class="page-title fw-semibold fs-18 mb-0">DPLC</h1>
                        <div class="ms-md-1 ms-0">
                            <nav>
                                <ol class="breadcrumb mb-0">
                                    <li class="breadcrumb-item"><a href="javascript:void(0);">Dashboard</a></li>
                                    <li class="breadcrumb-item active" aria-current="page">DPLC List</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                    ...
                    <div class="col-xl-12">
                        <div class="card custom-card">
                            <div class="card-header d-flex justify-content-between">
                                ...
                                <div>
                                <button type="button" class="btn btn-primary btn-wave btn-sm"  data-bs-effect="effect-flip-vertical" data-bs-toggle="modal" href="#modaldemo8"><i class='bx bx-plus'></i> Add Dplc</button>
                                <div class="modal fade" id="modaldemo8" tabindex="-1"
                                    aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h6 class="modal-title" id="exampleModalLabel">Create DPLC</h6>
                                                ...
                                                <form id="dplc_form">
                                                    <div class="mb-2">
                                                        <label for="recipient-name"
                                                            class="col-form-label">Dplc</label>
                                                        <input type="text" id="dplc_name" class="form-control form-control-sm" placeholder="Type Dplc" required>
                                                        <input type="hidden" id="token" value="{{ $token }}"/>
                                                    </div>
                                                    <div class="mb-2">
                                                        <label for="recipient-name"
                                                            class="col-form-label">City</label>
                                                            <select class="form-select" data-trigger name="cty_id" id="cty_id">
                                ...
                                <div class="modal fade" id="update_modal" tabindex="-1"
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h6 class="modal-title" id="exampleModalLabel">Update City</h6>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                    aria-label="Close"></button>
                                            </div>
                                            <form id="update_dplc_form">
...
$('#datatable-basic').on('click', '.update', function() {
    ...
    let id = $(this).data("id");
            ...
            $.ajax({
            url: '/update_dplc/'+id, // Your API endpoint
...
$('#datatable-basic').on('click', '.alert-confirm', function() {
        ...
        Swal.fire({
            ...
            if (e.isConfirmed) {
                ...
                let id = $(this).data("id");
                $.ajax({
                    url: baseUrl+'/dplcdelete/'+id, // replace with your API URL
                    method: 'POST',
                    data: {
...
$('#dplc_form').on('submit', function (e) {
            e.preventDefault();


            // Disable all inputs and buttons
          $('#dplc_form input, #dplc_form button').prop('disabled', true);
            $('#submitBtn').hide();
            $('#loadingBtn').show();
            ...
            // Prepare form data
            let formData = {
                dplc_name: $('#dplc_name').val(),
                cty_id: $('#cty_id').val(),
            };

            // Send AJAX request
            $.ajax({
                url: baseUrl+'/dplcadd',  // Replace with your API endpoint
                type: 'POST',
                ...
                success: function (response) {
                    ...
                    if (response.status === 1) {
                        showToast("bg-success","Dplc Added.",response.message)
                ...
                $('#dplc_form input, #dplc_form button').prop('disabled', false);
                $('#loadingBtn').hide();
                $('#submitBtn').show();
                 }
            });
...
$('#update_dplc_form').on('submit', function (e) {
            e.preventDefault();


            // Disable all inputs and buttons
          $('#update_dplc_form input, #update_dplc_form button').prop('disabled', true);
            $('#updateBtn').hide();
            $('#updateloadingBtn').show();

            // Prepare form data
            let formData = {
                dplc_name: $('#update_dplc_name').val(),
                cty_id: $('#update_cty_id').val(),
            };
            ...
            // Send AJAX request
            $.ajax({
                url: baseUrl+'/dplcupdate/'+id,  // Replace with your API endpoint
                type: 'POST',
                data: JSON.stringify(formData),
                contentType: 'application/json',
                headers: {
                ...
                success: function (response) {
                    ...
                    if (response.status === 1) {
                        showToast("bg-success","Dplc Updated.",response.message)
                ...
                $('#update_dplc_form input, #update_dplc_form button').prop('disabled', false);
                $('#updateloadingBtn').hide();
                $('#updateBtn').show();
                 }
            });
    ...
    // Destroy the existing DataTable instance (if exists)
              ...
              $('#datatable-basic').DataTable({
                            ...
                            "pageLength": 10,
                            "ajax": {
                                "url": baseUrl+"/dplc", // Replace with your API URL
                                "type": "GET",
                            ...
                            "columnDefs": [
                                ...
                                },
                                {
                                    ...
                                    "render": function(data, type, row, meta) {
                                        return `
                                            <button type="button" data-id="${row.id}" class="btn btn-warning btn-sm btn-wave update">Edit</button>
