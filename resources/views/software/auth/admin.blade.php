...
@section('content')
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
                        <h1 class="page-title fw-semibold fs-18 mb-0">Manager</h1>
                    ...
                    <div class="col-xl-12">
                        <div class="card custom-card">
                            <div class="card-header d-flex justify-content-between">
                                ...
                                <div>
                                 @if ($roles_id == 1)
                                    <button class="btn btn-primary btn-wave btn-sm" onclick="load_manager(2,{{$user_id}});">Admin</button>
                                @elseif ($roles_id == 2)
                                    <button class="btn btn-primary btn-wave btn-sm" onclick='load_manager(3,{{$user_id}});'>Franchise</button>
                                    <button class="btn btn-primary btn-wave btn-sm" onclick="load_manager(4,{{$user_id}});">Dealer</button>
                                    <button class="btn btn-primary btn-wave btn-sm" onclick="load_manager(5,{{$user_id}});">SubDealer</button>
                                    <button class="btn btn-primary btn-wave btn-sm" onclick="load_manager(6,{{$user_id}});">JuniorDealer</button>
                                @elseif ($roles_id == 3)
                                    <button class="btn btn-primary btn-wave btn-sm" onclick="load_manager(4,{{$user_id}});">Dealer</button>
                                    <button class="btn btn-primary btn-wave btn-sm" onclick="load_manager(5,{{$user_id}});">SubDealer</button>
                                    <button class="btn btn-primary btn-wave btn-sm" onclick="load_manager(6,{{$user_id}});">JuniorDealer</button>
...
$('#manager_table').on('click', '.alert-confirm', function() {
       ...
       let roles_id = {{ $roles_id }};
        ...
        if (roles_id === 1) {
            load_manager(2, user_id);
        } 
        else if (roles_id === 2) {
            load_manager(3, user_id);
        } 
        else if (roles_id === 3) {
            load_manager(4, user_id);
        } 
        else if (roles_id === 4) {
            load_manager(5, user_id);
        } 
        else if (roles_id === 5) {
            load_manager(6, user_id);
        }
        
        function load_manager(id,user_id){
    // Destroy the existing DataTable instance (if exists)
              ...
              $('#manager_table').DataTable({
                            ...
                            "pageLength": 10,
                            "ajax": {
                                "url": baseUrl+"/users/"+id+"/"+user_id, // Replace with your API URL
                                "type": "GET",
